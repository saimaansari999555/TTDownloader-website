...
| `72e811c` | Overhaul Website UI/UX: Plus Jakarta Sans Typography, Deep Obsidian Palette, and High-End SaaS Utilities | Website ko generic AI template aesthetic se transform karke professional human-crafted SaaS style me badla gaya: Plus Jakarta Sans font integrate kiya, harsh neon purple blobs ko sophisticated deep obsidian slate palette aur subtle ambient lighting se replace kiya, Video/Audio/Bulk downloader input boxes me quick clipboard paste button (`Ctrl+V`), clear button, aur trust micro-badges add kiye, navbar & footer borders ko refined kiya, aur Page Builder block renderers (Hero, Cards, Steps, FAQ) ko clean modern layout me convert kiya. |
| `f0a3e81` | Apply Stylish Modern Google Fonts (Urbanist & Space Grotesk) and Custom TikSavePro Favicon | Website me stylish geometric Google Fonts (`Urbanist` for body & `Space Grotesk` for headings/display) integrate kiye via `next/font/google` for ultra-clean rendering with zero CLS; default Vercel favicon ko replace karke custom TikSavePro brand icon (`public/icon.svg`, dynamic `app/icon.tsx`, and `app/apple-icon.tsx`) setup kiya. |
| `8a3df1e` | Fix Admin Panel Live Syncing, Cache-Busting, and Instant Client Settings Persistence | Admin Panel me changes save na hone aur website par show na hone ka issue resolve kiya: All API endpoints (`/api/settings`, `/api/pages`, `/api/blog/posts`, `/api/redirects`) par `force-dynamic`, `revalidate = 0`, aur `no-store` headers configure kiye; `lib/api.ts` me cache-busting timestamps (`_t=Date.now()`) add kiye; `useSettings` hook me instant `localStorage` cache aur `tiksave_settings_updated` live event sync add kiya; Admin Settings page par tab-level bottom Save action bar integrate kiya. |
| `372a819` | Upgrade Page Builder Editor with RichTextEditor, Media Library, Hero Controls, and Live Optimistic Updates | Page Builder editor (`/admin/pages`) ko Blog editor ke pattern par overhaul kiya: Buggy block inputs ko full WYSIWYG `RichTextEditor` se replace kiya (H1-H3 headings, lists, links, quotes, ChatGPT paste converter, code mode), Hero banner controls, Tool embed selector, Featured image upload with canvas compression, Media Library modal, Google SERP SEO preview, instant optimistic `localStorage` cache, aur live event refresh (`tiksave_pages_updated`) add kiya. |
| `94fb21c` | Temporarily Remove Header, Footer, and Result Advertisement Banners | Website pages (`/p/[slug]`), `VideoDownloader`, aur `AudioDownloader` se header top, footer bottom, aur result ad banner placeholders ko temporarily remove kiya aur default settings ad codes ko empty clear kiya. |
| `187d9ef` | Restructure Website URLs: Consolidate Video Downloader on Home Route (/) and Establish Canonical URLs | User requirements ke mutabiq URLs restructure kiye: Video Downloader ko direct Home page (`https://tik-tokdownloader.xyz/`) par consolidate kiya aur `/video` se 301 redirect lagaya; Audio Extractor ka canonical URL `/audio-extractor` banaya (`/audio` -> 301); Bulk Downloader ka canonical URL `/bulk-downloader` banaya (`/bulk` -> 301); Android APK ka canonical URL `/android-apk` banaya (`/apk` -> 301); Navbar, Footer, Sitemap.xml, aur CMS Presets ko naye clean URLs se update kiya without losing any page content. |
| `0f2a961` | Fix Admin Panel Settings Persistence and Overwrite Bug | `useSettings` hook aur Admin Settings page me persistence architecture fix kiya: Serverless defaults se client `localStorage` settings ke wipe hone ka bug resolve kiya (user saved settings ko priority di), instant cache hydration lagaya aur live event synchronization integrate kiya. |
